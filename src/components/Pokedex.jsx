...
const Pokedex = () => {
  ...
  useEffect(() => {
    axios.get('https://pokeapi.co/api/v2/pokemon?offset=1&limit=1200').then((res) => {
  ...
  const getTypePokemons = (url) => {
    if (url) {
      ...
      axios.get('https://pokeapi.co/api/v2/pokemon?offset=1&limit=1200')
        .then(res => {
          setPokemons(res.data.results)
          setPage(1)
        })
  ...
  return (
    <section className='p-10 lg:px-24 xl:pr-32 xl:pl-8 2xl:pr-36 2xl:pl-20'>
      <nav>
        ...
        <h1 className='font-head font-bold text-7xl md:text-8xl 2xl:text-9xl inline'>Pokedex</h1>
        ...
        <select className='bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-black focus:border-black block 
        w-40 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 
        dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 mt-10' onChange={(e) => getTypePokemons(e.target.value)}>
        ...
        <div className='absolute'>
          ...
          overflow-x-hidden overflow-y-auto' src={poke_bg_gray} alt=''
          />
        </div>
      </nav>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-y-10 gap-x-24  my-14'>
        {
            paginatePokemons?.map((pokemon) => (
              <Link to={`/pokedex/${pokemon.name ? pokemon.name : pokemon?.pokemon?.name}`} key={pokemon.url ? pokemon.url : pokemon?.pokemon?.url}>
      ...
      <div className=''>
        ...
        <span className='    flex justify-center pt-10 '> {page} / {totalPages} </span>
        ...
        <div className="flex flex-col items-center">
          ...
          {
             page !== 1 &&
            <button onClick={() => setPage(page - 1)} className="py-2 px-4 text-sm font-medium text-white bg-gray-800 rounded-l 
          ...
          {  page !== totalPages && (
          <button onClick={() => setPage(page + 1)} className="py-2 px-4 text-sm font-medium text-white bg-gray-800 rounded-r border-0 border-l 
